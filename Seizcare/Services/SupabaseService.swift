...
final class SupabaseService {
    ...
    func insertSensitivity(dto: SensitivityDTO) async throws {
        try await client
            .from("user_sensitivity")
            .insert(dto)
            .execute()
    ...
    func updateSensitivity(dto: SensitivityDTO) async throws {
        try await client
            .from("user_sensitivity")
            .update(dto)
    ...
    // MARK: - User Language Table

    func fetchLanguage(userId: UUID) async throws -> LanguageDTO? {
        let rows: [LanguageDTO] = try await client
            .from("user_language")
            .select()
            .eq("user_id", value: userId.uuidString)
            .limit(1)
    ...
    func insertLanguage(dto: LanguageDTO) async throws {
        try await client
            .from("user_language")
            .insert(dto)
            .execute()
    ...
    func updateLanguage(dto: LanguageDTO) async throws {
        try await client
            .from("user_language")
            .update(dto)
            .eq("user_id", value: dto.userId.uuidString)
...
struct SensitivityDTO: Codable {
    ...
    init(userId: UUID, sensitivityLevel: String) {
        self.id = nil // UUID is generated by Supabase default or omitted on insert/update
        self.userId = userId
...
struct LanguageDTO: Codable {
    ...
    init(userId: UUID, languageCode: String) {
        self.id = nil
        self.userId = userId
